...
test.describe('Pagination', () => {
  ...
  test('should default to the first page', async ({ page }) => {
    await page.goto(baseUrl);
    await expect(page.getByLabel('Pagination Navigation').locator('a')).toContainText('Page 1');
  });
  ...
  test('should navigate to the next page', async ({ page }) => {
    ...
    await expect(page.getByLabel('Pagination Navigation').locator('a')).toContainText('Page 2');
  });
  ...
  test('should update the posts list on page change', async ({ page }) => {
    await page.goto(baseUrl);
    const firstPostOnPage = await page.locator('[data-testid=post-item] .card-message' ).nth(0).allInnerTexts();
    await page.getByLabel('Go to next page').click();
    await page.waitForURL(`${baseUrl}/?page=2`);
