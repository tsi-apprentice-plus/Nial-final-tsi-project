...
test.describe("Search", () => {
  ...
  test("should update the posts list based on search query", async ({
    page,
  }) => {
    await page.goto(baseUrl);
    const search =
      "Vivamus in felis eu sapien cursus vestibulum. Proin eu mi. Nulla ac enim.";
    const existingPosts = await page
    ...
    await page.waitForURL(
      `${baseUrl}/?page=1&query=Vivamus+in+felis+eu+sapien+cursus+vestibulum.+Proin+eu+mi.+Nulla+ac+enim.`,
    );
